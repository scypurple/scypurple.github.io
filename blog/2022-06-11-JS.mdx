...
+ `this`和词法作用域的查找混合使用是无法实现的，`this`在任何情况下都不指向函数的词法作用域。作用域无法通过 js 代码访问，它存在域 js 引擎内部
+ `this`的绑定和函数声明的位置没有任何关系，只取决与函数的调用方式

#### this 到底是什么

+ 当一个函数被调用时，会创建一个活动记录（有时候也叫执行上下文）
  + 这个记录会包含函数在哪里被调用（调用栈）
  + 函数的调用方式
  + 传入的参数等信息
...
+ 默认绑定：独立函数调用，非严格模式 全局对象，严格模式 undefined
+ 隐式绑定：调用位置是否有上下文对象
+ 显示绑定：call apply bind
+ new 绑定：在 JS 中构造函数只是一些使用`new`操作符时被调用的函数
...
+ 能组合就不继承（自己创建类，方法可以委托给其他类现有的方法）

## 模块

#### 模块化原理 

+ 利用函数闭包将要暴露的值作为函数的返回值赋给变量
...
+ 右边变量的`prototype`在左边变量的`_proto_`原型链上即可
